...
Supply challenge.test with the actual HTTP or browser method, tested input, and a specific string, scalar, or numeric result assertion. It must pass on two separate captures and fail on a meaningful control. Prefer the same endpoint; an alternate endpoint on the same authorized origin is allowed when control_strategy is equivalent and control.purpose explains the comparison. Scanner summaries and legacy receipts without request metadata cannot serve as reproduction proof. Keep potential impact in inferred_impact; no database write claim follows from injection detection. Link each completed planned check to its evidence IDs. Blocked and inconclusive checks remain gaps when the report closes the engagement.
